...
router.get("/config", optionalApiKey, async (req, res) => {
  res.json({
    ...
    zcash_devtool: {
      ...
      description:
        "Official Zcash Foundation tool for quick CLI-based testing of wallets and transactions",
      version: "latest",
      repository: "https://github.com/zcash/zcash-devtool",
      documentation:
        "https://github.com/zcash/zcash-devtool/blob/main/doc/walkthrough.md",
      ...
      age_setup: {
        install:
          "# Install age: brew install age (macOS) or apt install age (Ubuntu)",
...
router.get("/wallet/user/:user_id", optionalApiKey, async (req, res) => {
  ...
  try {
    ...
    const wallets = result.rows.map((wallet) => ({
      ...
      wallet_path: wallet.wallet_path,
      server_url:
        wallet.network === "mainnet" ? "zec.rocks" : "zec-testnet.rocks",
      created_at: wallet.created_at,
    }));
  ...
  try {
    ...
    res.json({
      ...
      commands: {
        ...
        troubleshooting: {
          "Sync issues":
            "Ensure network connection and try different light server",
          "Age key errors":
            "Make sure identity.age exists and AGE_FILE_SSH_KEY=1 is set",
  ...
  try {
    ...
    if (wallet_id) {
      ...
      if (walletResult.rows.length === 0) {
        return res
          .status(404)
          .json({ error: "zcash-devtool wallet not found" });
      }
    ...
    const serverUrl =
      walletInfo?.network === "mainnet" ? "zec.rocks" : "zec-testnet.rocks";
...
router.get("/guide", optionalApiKey, async (req, res) => {
  res.json({
    ...
    zcash_devtool_guide: {
      overview:
        "zcash-devtool is an official Zcash Foundation CLI tool for prototyping wallet functionality without running a full node.",
      ...
      common_commands: {
        wallet_management: {
          "Initialize wallet":
            "cargo run --release -- wallet -w <path> init --name <name> -i ./identity.age -n <network>",
          "Sync wallet":
            "cargo run --release -- wallet -w <path> sync --server <server>",
          "Check balance": "cargo run --release -- wallet -w <path> balance",
          "List addresses": "cargo run --release -- wallet -w <path> addresses",
          "Generate address":
            "cargo run --release -- wallet -w <path> new-address",
          "List transactions":
            "cargo run --release -- wallet -w <path> list-txs",
