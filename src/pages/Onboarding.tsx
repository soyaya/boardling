...
const Onboarding: React.FC = () => {
  ...
  const { currentStep, previousStep, isCompleting, setStep } = useOnboardingStore();
    ...
    const restoreOnboardingState = async () => {
      if (isAuthenticated && user && !user.onboarding_completed) {
        ...
        if (user.has_project || user.has_wallet) {
          // Project or wallet exists - go to step 2 (project/wallet form)
          console.log('Resuming onboarding: Project/wallet exists, showing form');
          
          // Fetch the existing project ID if it exists
          if (user.has_project) {
            try {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center p-6">
        <AnimatePresence mode="wait">
          {currentStep === 1 && !isCompleting && <WelcomeStep key="step1" />}
          {currentStep === 2 && !isCompleting && <ConnectWalletStep key="step2" />}
        ...
        <div className="p-8">
          <div className="max-w-md mx-auto">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-xs text-gray-500">{Math.round((currentStep / 2) * 100)}% Complete</span>
            </div>
